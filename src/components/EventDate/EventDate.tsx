...
function convertTZ(date: Date | string | number, tzString: string) {
    return new Date(
        (date instanceof Date ? date : new Date(date)).toLocaleString('en-US', {
            timeZone: tzString,
        })
    )
}
...
export function EventDate(props: {
    ...
    const { startAndFinishSameDay, inCurrentYear } = useEventDateHelper(
        props.startAt,
        props.finishAt
    )
...
export function useEventDateHelper(startAt: Date, finishAt: Date) {
    ...
    const currentDate = intl.timeZone
        ? convertTZ(new Date(), intl.timeZone)
        : new Date()
    const convertedStartAt = intl.timeZone
        ? convertTZ(startAt, intl.timeZone)
        : startAt
    const convertedFinishAt = intl.timeZone
        ? convertTZ(finishAt, intl.timeZone)
        : finishAt
