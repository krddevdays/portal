...
export async function createOrder(
    ...
    const seatsInfo = tickets.types.reduce<Record<string, EventTicketsType>>(
        (seats, type) => {
            if (!seats[type.id]) {
                seats[type.id] = type
            }
    ...
    for (const ticket of order.data.tickets.items) {
        ...
        if (seat.free_quantity < selectedSeatsByType[ticket.type].length) {
            // TODO
            notFound()
        }
    }
    ...
    const requestBody = {
        data: {
            ...
            payment_type_id: payment.type === 'card' ? 'card' : undefined,
            fields:
    ...
    try {
        const result = await qticketsApi.post(requestBody, '/orders').json<{
            data: {
                id: number
                payment_url: string
                reserved_to: string
